...
```bash
sedes-api-serverless$ sam build
```
...
```bash
sedes-api-serverless$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
sedes-api-serverless$ sam local start-api
sedes-api-serverless$ curl http://localhost:3000/
```
...
```bash
sedes-api-serverless$ sam build
sedes-api-serverless$ sam deploy
```
...
```bash
sedes-api-serverless$ sam logs -n NetCodeWebAPIServerless --stack-name sedes-api-serverless --tail
```
...
```bash
sedes-api-serverless$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name sedes-api-serverless
```
